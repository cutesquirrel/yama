...
module.exports = {
    ...
    // - destroy    : scalar or function.
    //                If scalar  : true to remove the document from the collection when DELETE is used,
    //                             false to set an active field to 0 in this document (useful for users).
    //                If function: has to be a function returning a boolean.
    //                             Can be used to test a permissions for example, depending of the logged-in user.
    //                             `function(req){ return boolean; }`
    //                Default : `false`.
    ...
    //     -  `field_name` :
    //            + label  : label of the field (previously called `header`).
    //            + help   : help displayed under the field, to give more details.
    ...
    //         if (err) callback(err, fieldName, req.body);
    //
    //         if (result) {
    //           req.body[fieldName] = {
    //             _id: result._id,
    ...
    // };
    // ````
    // - after_save : hook called after saving the document.
    // ````
    // after_save: function(req, callback) {
    //    // Do what you want and finish by calling the callback.
    // }
    // ````
    //
    add: function(path, modelName, schema, opts) {
        ...
        // Checkup of opts
        var allUsedFields = _.union(opts.list, opts.edit, opts.filters);

        for (var idx in allUsedFields) {
            var f = allUsedFields[idx];

            if (!opts.fields.hasOwnProperty(f)) {
                console.error('YAMA ERROR, admin.add: field ' + modelName + '.' + f + ' isn´t defined in your settings.');
            }
        }
