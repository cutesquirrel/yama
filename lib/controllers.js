...
var addHelpers = function(req, res) {
    ...
    res.locals.isType = function(param, typeString) {
        if (typeString === "object" && Array.isArray(param)) {
            return false;
        }
...
controllers.list = function(req, res) {
    ...
    if (req.user) {
        ...
        _.each(meta.fields, function(f, key) {
            if (!f.label) {
                // old "header" compatibility, key either.
                f.label = f.header || key;
            }
        });
...
controllers.save = function(req, res) {
    ...
    if (req.user) {
        ...
        Model.findOne({
            ...
            helpers.processFormFields(meta, req, req.body, function(err) {
                if (err) {
                ...
                doc.save(function(err, result) {
                    ...
                    // If an after_save postprocessor exists.
                    if (meta.after_save && typeof(meta.after_save) === 'function') {
                        meta.after_save(req, function(err, fname, val) {
                            if (err) {
                                console.log('controllers.save: Error in after_save postprocess function of field %s', fieldName, ', values:', val, ', error:', err);
                                req.flash('error', 'after_save(' + fieldName + ')' + err.message);
                                return res.redirect('back');
                            }
