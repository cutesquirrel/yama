...
helpers.capitalizeFirstLetter = function(string) {
    ...
    console.log(string, typeof(string), string.charAt(0));
...
helpers.processEditFields = function(type, meta, doc, cb) {
    ...
    // ------------------------
    // Processing fields which use `select` widget.
...
helpers.processFormFields = function(meta, req, values, cb) {
    ...
    for (f in meta.edit) {
        ...
        // if "ref with mongoose's populate" field or "select from model" field or checkbox
        if (values[fieldName] === 'true' || values[fieldName] === 'false' || values[fieldName] === '' || meta.fields[fieldName].widget === 'ref' || meta.fields[fieldName].ref || meta.fields[fieldName].before_save) {
            fieldProcesses.push(meta.edit[f]);
            count++;
        }
    }
    //console.log(values);
    //console.log('fieldProcesses', fieldProcesses);
    ...
    async.eachLimit(fieldProcesses, 20, function(fieldName, done) {
            ...
            } else if (values[fieldName] === 'false') {
